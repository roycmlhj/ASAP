/* 작성자 : 한슬기 생성일 : 2022.01.30 마지막 업데이트 : 2022.01.30 개인정보수정
페이지 */
<template>
  <section>
    <div class="container">
      <b-card bg-variant="light" class="card mb-4">
        ...
        <img src="/resources/클래스다이어그램.jpg" alt="" />
        <p>{{ this.user.email }}</p>
        <a href="#" class="float-right" @click="userDelete">회원탈퇴</a>
      </b-card>
      <b-card bg-variant="light" class="input">
        <b-form-group label="비밀번호" label-for="password" label-cols-sm="1">
          <b-form-input
            id="password"
            type="password"
        ...
        <b-form-group label="닉네임" label-for="nickname" label-cols-sm="1">
          <b-form-input
            id="nickname"
      ...
      <div class="d-flex mt-4">
        <b-card class="col-6" bg-variant="light">
          ...
          <div class="d-flex justify-content-between mt-3">
            ...
            <b-button
              @click="imageUpload()"
              style="background-color: skyblue; border: none"
              >Add</b-button
            >
          </div>
          <b-form-checkbox
      ...
      <b-button class="btn1 mt-5 float-right" @click="userEdit"
        >저장하기</b-button
      >
    </div>
  </section>
  ...
  computed: {
    ...
    passwordState() {
      ...
      if (this.user.password == 0) {
        ...
        !pattern1.test(this.user.password) ||
        !pattern2.test(this.user.password) ||
        !pattern3.test(this.user.password) ||
